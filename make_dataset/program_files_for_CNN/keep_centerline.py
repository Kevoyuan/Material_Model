...
main_dir = 'roughmodel_1024/dataset_inp'

# Get total number of files
total_files = sum(len(files) for _, _, files in os.walk(main_dir))/3
...
with tqdm(total=total_files, unit='file') as pbar:
    ...
    for dirpath, dirnames, filenames in os.walk(main_dir):
        for filename in filenames:
            # Check if the file is a CSV and its name starts with "dataset_inp_57_"
            if filename.endswith('.csv') and filename.startswith('dataset_inp_58_'):
