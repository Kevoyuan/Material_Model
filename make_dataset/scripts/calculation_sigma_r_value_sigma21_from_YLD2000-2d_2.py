...
def main():
    ...
    parameter = [9.446146322,1.099164512,1.169133037,1.158363545,1.027356956,1.026355236,1.189080111,1.094422696,1.110759445]
    ...
    # 平面ひずみ応力点の計算
    cal_point_plane_strain = [26.56505118,63.43494882] #sigma_x:sigma_y=2:1 and 1:2
    sig_x_y_plane_strain = []
    for i in cal_point_plane_strain:
        #二軸
        ratio = np.tan(np.radians(i))
        parameter1 = [ratio] + parameter

        #sig_x_yでの降伏曲面
        sig_x_y_plane_strain.append(cal_sig_x_y(*parameter1))
    ...
    a_sig_ps21_x=sig_x_y_plane_strain[0][0]
    a_sig_ps12_y=sig_x_y_plane_strain[1][1]
    a_output = np.hstack([a_unixial_stress, a_sig_b, a_r_value, a_r_b, a_M, a_sig_ps21_x, a_sig_ps12_y]) 

    output = [['sig_00', 'sig_45', 'sig_90', 'sig_b', 'r_00', 'r_45', 'r_90', 'r_b', 'M','sig_ps21_x', 'sig_ps21_y']]
    output += [a_output.tolist()]
    print(output)
    write_csv('./output_sigma_r-value_230131.csv', output)
    return
